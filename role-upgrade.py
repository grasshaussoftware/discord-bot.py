...
class button_view(discord.ui.View):
    ...
    async def get_mod(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if interaction.user.get_role(0) is None:
            role = interaction.guild.get_role(0) or await interaction.guild.fetch_role(0)
            await interaction.user.add_roles(role)
            await interaction.response.send_message(f"I have granted you {interaction.guild.get_role(0)}!", ephemeral = True)
        else:
            await interaction.response.send_message(f"You already possess {interaction.guild.get_role(0)} or HIGHER!", ephemeral = True)
